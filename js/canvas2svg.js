/* canvas2svg 1.0.16 (gliffy/canvas2svg) — LOCAL PATCHED COPY.
   Patch: strokeStyle = <gradient> is now emitted. Upstream only handled fillStyle
   gradients; a stroked gradient (e.g. contact-map ribbons) fell through to
   value.indexOf("rgba") and threw "value.indexOf is not a function". */
/*!!
...
;(function() {
        ...
        for(i=0; i<keys.length; i++) {
            ...
            if(style.apply) {
                ...
                if(style.apply.indexOf("fill")!==-1 && value instanceof CanvasPattern) {
                    ...
                    this.__currentElement.setAttribute(style.svgAttr, format("url(#{id})", {id:value.__root.getAttribute("id")}));
                }
    ...
    ctx.prototype.getSerializedSvg = function(fixNamedEntities) {
        ...
        // alpha; hex colors and ids have no decimals so they are untouched. Cuts
